...
async def python_analyze_node(state: dict) -> dict:
    ...
    max_repair_attempts = 2 if should_use_llm_python_generate(state) else 0
    current_code = code
    for attempt_index in range(max_repair_attempts + 2):
        ...
        if executed and executed.ok:
            ...
            if current_code != code:
                return {"python_code": current_code, "python_result": payload}
            return {"python_result": payload}

        await emit_phase3_stream(
            "python_analyze",
            f"\n第 {attempt_index + 1} 次 Python 分析执行失败，正在观察错误并尝试修复脚本。\n错误摘要：{compact_error(error_text)}\n",
            chunk_size=80,
        )
        if attempt_index < max_repair_attempts:
            repaired = await repair_python_code_with_llm(state, profile, current_code, error_text, attempts)
            if repaired:
                current_code = repaired
                continue
        template_code = _build_analysis_code(mode)
        if current_code != template_code:
            await emit_phase3_stream("python_analyze", "\n已切换到安全模板脚本重新执行。\n", chunk_size=80)
    ...
    failed_payload = {
        "status": "failed",
        "row_count": len(rows),
        "error": compact_error(attempts[-1].get("error") if attempts else "Python 分析失败"),
        "analysis_scope": "SQL 结果集分析，不访问业务库",
        "analysis_mode": mode,
    ...
    return {"python_result": failed_payload}
...
def execute_python_analysis_attempt(code: str, rows: list[dict[str, Any]]) -> tuple[Any | None, str]:
    try:
...
def python_success_payload(executed: Any, state: dict, attempts: list[dict[str, Any]]) -> dict[str, Any]:
...
async def repair_python_code_with_llm(
    ...
    try:
        ...
        variables.update({
            ...
            "python_error": compact_error(error_text, limit=2400),
        ...
        user_prompt = (
            ...
            "再重新输出一份完整、可执行、安全的 Python 代码。不要解释，不要 Markdown，只输出代码。\n\n"
        ...
        await emit_phase3_stream(
            "python_analyze",
            f"\n脚本修复生成失败，准备使用安全模板兜底。原因：{str(exc) or exc.__class__.__name__}\n",
            chunk_size=80,
        )
        return ""
...
def infer_analysis_mode(state: dict, profile: dict[str, Any]) -> dict[str, str]:
    time_dimensions, category_dimensions = _split_time_like_dimensions(profile.get("dimension_columns") or [])
...
def _build_analysis_code(mode: str = "profile") -> str:
    template_name = "multi_series_trend.py.tpl" if mode == "multi_series_trend" else "generic_analysis.py.tpl"
...
def _build_report_payload(
    ...
    executive_summary = _report_executive_summary(rows, metric_keys, dimension_keys, python_result, mode)
    ...
    interpretation = _report_interpretation(rows, profile, metric_keys, dimension_keys, python_result, mode)
    ...
    return {
        "title": title,
        "summary": summary,
        "status": "analysis_failed" if python_result.get("status") == "failed" else ("empty" if not rows else "success"),
        "mode": mode,
...
def _report_summary(
    ...
    if python_result.get("status") == "failed":
        return f"SQL 查询返回 {len(rows)} 行数据，但 Python 深度分析在重试后仍未成功：{python_result.get('error', '未知错误')}。"
    if mode == "multi_series_trend":
        insights = [str(item) for item in (python_result.get("insights") or []) if str(item or "").strip()]
...
def _report_highlights(rows: list[dict[str, Any]], python_result: dict[str, Any], mode: str = "profile") -> list[dict[str, Any]]:
    ...
    if mode == "multi_series_trend":
        ...
        if isinstance(series_summary, list):
            highlights.append({"label": "趋势序列数", "value": len(series_summary), "field": "series_count"})
        return highlights
...
def _report_process(
    ...
    if python_result.get("status") == "failed":
        ...
        steps[1]["text"] = "Python 分析脚本执行失败，系统已按 ReAct 思路观察错误并尝试重新生成脚本。"
        steps[1]["result"] = f"已尝试 {len(attempts)} 次，最终失败原因：{python_result.get('error', '未知错误')}"
...
def _report_suggestions(
    ...
    if mode == "multi_series_trend":
        ...
        items.append("建议进一步补充渠道、区域或风险等级维度，观察不同贷款产品趋势是否存在结构性分化。")
...
def report_to_stream_text(report: dict[str, Any]) -> str:
    ...
    if isinstance(python_result, dict) and python_result.get("status") == "failed":
        ...
        attempts = python_result.get("repair_attempts") or []
        if attempts:
            lines.append(f"- 系统已按 ReAct 思路尝试观察错误、重新生成脚本并再次执行，共尝试 {len(attempts)} 次。")
        lines.append("- 当前报告仅保留 SQL 返回结果、错误说明和复核建议，暂不输出 Python 统计图表。")
