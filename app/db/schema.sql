...
CREATE TABLE IF NOT EXISTS ontology_object_type (
    ...
    sync_enabled TINYINT(1) DEFAULT 0 COMMENT '是否启用业务库同步',
    source_query LONGTEXT DEFAULT NULL COMMENT '对象实例只读同步SQL',
    sync_limit INT DEFAULT 200 COMMENT '单次分页同步行数',
    last_sync_status VARCHAR(32) DEFAULT NULL COMMENT '最近同步状态',
    last_sync_count INT DEFAULT 0 COMMENT '最近同步读取行数',
    last_sync_total INT DEFAULT 0 COMMENT '业务库对象总数',
    last_sync_error TEXT DEFAULT NULL COMMENT '最近同步错误',
    last_synced_at TIMESTAMP NULL DEFAULT NULL COMMENT '最近同步时间',
...
CREATE TABLE IF NOT EXISTS ontology_object (
    ...
    source_kind VARCHAR(32) DEFAULT 'manual' COMMENT 'manual/bundle/database',
    source_datasource_id BIGINT DEFAULT NULL COMMENT '来源数据源ID',
    source_properties JSON DEFAULT NULL COMMENT '业务库同步属性快照',
    overlay_properties JSON DEFAULT NULL COMMENT '本体动作本地覆盖属性',
    last_synced_at TIMESTAMP NULL DEFAULT NULL COMMENT '最近业务库同步时间',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS ontology_action_run (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    domain_id BIGINT NOT NULL COMMENT '所属语义领域',
    ontology_release_id BIGINT DEFAULT NULL COMMENT '动作执行绑定的Ontology发布版本',
...
CREATE TABLE IF NOT EXISTS ontology_release (
    ...
    definition_hash CHAR(64) DEFAULT NULL COMMENT '不可变发布定义SHA-256',
