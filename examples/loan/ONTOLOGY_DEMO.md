...
这里的“贷款申请”指的是一笔具体的申请单/进件记录，不是抽象的“贷款审批流程”。判断一个概念是否建成对象，主要看它是否有独立业务标识、自己的属性、生命周期、对象关系，以及是否需要被查询、复核或执行动作。

在本 Demo 中：

| 概念 | 表达方式 | 示例 |
|---|---|---|
| 申请单本身 | 对象类型 | `LoanApplication` |
| 某一笔申请 | 对象实例 | `APP-20260830-0001` |
| 当前审批阶段 | 状态/属性 | `approval_status=manual_review` |
| 审批或拒绝 | 动作 | `approve_application` |
| 提交、通过、放款 | 事件 | `ApplySubmitted`、`ApplicationApproved`、`LoanDisbursed` |

因此，图谱中的“贷款申请单”应理解为“会经历审批过程的业务申请单”；“申请”“进件”“贷款申请”仍可作为它的业务叫法。流程用状态、事件和动作表达，不把抽象流程定义直接当成申请对象。
