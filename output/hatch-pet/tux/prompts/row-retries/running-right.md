Create Codex pet row `running-right` for `tux`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A black-and-white tuxedo cat with a white chest patch, white paws, bright yellow-green eyes, and a cozy domestic personality.. Preserve silhouette, face, palette, material, proportions, markings, and props.
