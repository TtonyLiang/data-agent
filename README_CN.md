...
> 以企业本体建模为核心的智能平台 —— 统一定义对象、关系、状态与动作，连接智能问数、决策执行与全链路审计。
...
通过对象类型、属性、关系、状态和业务动作描述企业业务世界，支持模型校验、版本发布、对象实例、权限约束和运行审计。企业本体为数据、AI Agent 和业务应用提供统一的业务模型。
...
业务动作作为本体模型的一部分，显式定义参数、前置条件、授权角色和状态效果。每次执行记录决策上下文、执行前后对象快照与状态变化，形成可追溯的业务闭环。
...
系统由企业本体建模工作台、智能问数界面、FastAPI 后端、LangGraph 工作流、本体/语义运行时和外部依赖（LLM、MySQL、Milvus）组成。前端通过 REST API 和 SSE 与后端交互，后端统一承载本体发布、问数分析和受控决策执行链路。
...
企业本体提供对象、关系、状态和动作模型；查询语义补充概念、指标、规则、映射和模板，并把本体连接到真实数据与可编译查询。
...
企业本体是问渠 WenQu 的核心模型：通过对象、属性、关系、状态和动作表达企业业务世界；查询语义负责将这套模型连接到指标口径、物理字段和可编译查询，使数据答案能够进入受控、可审计的业务行动。
