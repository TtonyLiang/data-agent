...
- `KnowledgeConfig`：以 `business/binding` 两种模式嵌入企业模型。业务模式按本体对象组织查询词汇、指标和业务规则；绑定模式按对象来源、指标计算、关系连接、字段映射和高级查询配置组织底层语义资产。
- `ObjectDataBindingPanel`：从已采集 Schema 选择业务表和字段，自动生成对象同步只读查询；复杂场景保留高级 SQL，并提供不写入的映射预览。
