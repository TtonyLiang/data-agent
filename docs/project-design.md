...
问渠 WenQu 收敛为一个**面向财税服务的 AI 报告交付与风险决策平台**。平台帮助财税服务团队把数据和问数结果转化为风险事项，关联可追溯证据，经过人工复核后形成版本化报告，并触发受控决策动作与全链路审计。

客户购买的是报告交付效率、风险事项覆盖、证据可追溯和决策可审计，而不是通用本体建模工具。Ontology 继续保留对象、属性、关系、状态、动作、权限和版本等通用能力，但定位为底层业务模型，连接数据、指标、规则、证据、复核和动作。
...
- 用 Ontology 表达业务对象、属性、关系、状态和可执行动作，并通过校验、发布和版本留痕治理模型。
...
- 当前可生成可展示、可恢复的分析报告；产品切片继续补充风险事项、证据、人工复核和不可变报告版本。
...
| 状态 | 能力范围 |
|---|---|
| 已实现基础 | 智能问数、语义资产、LogicForm 校验与确定性 SQL 编译、Python 分析、报告生成、持久任务；通用 Ontology 对象/关系/动作 CRUD、校验发布、实例运行和动作审计原型 |
| 已实现技术切片 | 风险事项、证据关联、指派复核与禁止自审、不可变报告版本，以及报告、Ontology 发布和决策动作之间的哈希链审计关联 |
| 后续业务验证 | 真实财税规则、重要性标准、证据充分性、复核角色与权限、正式报告模板、真实案例 UAT 和首个付费试点 |

内部财税业务人员加入前，当前新增能力先使用贷款风控域做技术验证。贷款数据、阈值、规则和结论均为合成演示，不构成真实授信、财务、税务、会计、审计或其他合规意见。
...
flowchart LR
  ...
  ONTOLOGY["Ontology 底层业务模型<br/>对象 / 关系 / 状态 / 动作 / 权限 / 版本"] -.-> SOURCE
  ONTOLOGY -.-> ISSUE
  ONTOLOGY -.-> REVIEW
  ONTOLOGY -.-> ACTION
```

截至 2026-09-01，贷款技术切片已覆盖风险事项、证据、人工复核、不可变报告版本、受控动作和决策审计；真实财税规则、职责角色、正式模板和客户价值仍属于后续业务验证。
...
风险报告交付能力复用现有鉴权、语义运行时、任务 checkpoint、报告生成和 Ontology 动作能力，并在其上增加领域记录与业务状态流转。它不改变现有问数链路的安全边界，也不要求先建设完整企业知识图谱。
...
- Ontology 是平台底层业务模型；通用 CRUD 作为配置与扩展能力保留，不作为客户购买的最终产品。
- 产品开发围绕一个可衡量的报告交付闭环推进，不以对象类型数量或本体规模作为成功指标。
...
- 演示域中的规则和阈值必须明确标记为技术样例；只有经过业务负责人确认和真实案例 UAT 的规则才能作为财税业务规则。
...
- 当前产品主线是完成风险事项、证据、人工复核、报告版本和决策审计的贷款技术切片，再由内部财税团队选择一个真实交付流程进入业务验证。
...
详细范围、入口条件和验收指标见[风险报告交付产品路线图](./risk-report-delivery-roadmap.md)。阶段摘要如下：

| 阶段 | 核心目标 | 关键验收指标 |
|---|---|---|
| 贷款技术切片 | 以合成贷款案例验证完整技术契约 | 至少 10 个端到端案例；证据、复核、报告版本和审计关联完整率 100% |
| 内部财税流程选择 | 选择一个真实且高频的内部交付流程 | 1 份确认的决策画布、不少于 20 个历史案例、基线指标、首批规则和报告模板 |
| 历史案例影子运行 | 与专家原流程并行验证效率和边界 | 不少于 30 个案例、100% 人工复核、不低于 95% 证据追溯率、准备时间降低至少 30% |
| 首个付费试点 | 验证客户愿意为交付结果付费 | 1 个付费范围、至少 4 个交付批次、审计完整率 100%，并形成续费/扩围/停止决定 |
