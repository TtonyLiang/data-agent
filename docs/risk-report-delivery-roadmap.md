...
本文只描述“风险报告交付”这一垂直应用如何验证平台价值，不再代表整体产品路线。贷款风控是合成技术样例，财税报告是候选真实场景；内置验证 Agent 和其他第三方 Agent 都可以复用相同底座。
...
截至 2026-09-04，文档按以下三类描述能力，避免把目标状态写成现状：
...
| P0 平台兼容骨架 | 业务领域归属、内置验证 Agent 适配、企业模型/孪生运行/能力中心入口；不等同于生产级运行时或网关 |
...
2. 先完成业务领域兼容、企业模型统一入口、孪生运行入口和能力中心入口。
3. 补齐真实数据的增量同步、身份解析、质量与血缘，再选择一个真实业务领域。
4. 将该领域的 Query / Decision / Action 以明确版本和权限发布，并分别用内置验证 Agent 和一个外部调用样例验收。
