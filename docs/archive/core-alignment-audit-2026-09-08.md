# [历史] 核心目标一致性扫描与整改清单（2026-09-08）

> 核心目标：**让业务语义成为企业资产，让数据库成为数据来源，让 Agent 成为能力消费者。**
>
> 本清单是 2026-09-08 整改工作的历史快照，不再维护当前状态。平台路线、阶段状态和最终验收以 [Ontology 产品路线图](../ontology-product-roadmap.md) 为准。
...
| CORE-004 | P0 | 外部能力授权曾要求技术人员选择内部验证 Agent 作为权限执行适配 | `CapabilityGrantUpsertPayload`、`CapabilityPublishCenter.vue` | 授权只选择领域和能力；旧 Agent 适配由平台内部兼容 | **已修复** |
...
范围校正：现有 `admin/user` 和用户到验证 Agent 的授权只用于当前系统管理、演示和回归。平台角色固定为业务人员与技术人员；当前开发者暂用 `admin` 完成两类操作，本轮不新增工作模式或独立的兼容账号领域授权，不把验证账号体系扩张为平台主线。
...
以上结果验证的是合成数据和自动化基线，不替代业务确认人（由业务人员承担）对对象、指标口径、数据权限和标准问题集的验收。
