...
| CORE-011 | P1 | 对象、指标、关系和映射页面仍让业务建模人员直接面对表、字段和 SQL | `OntologyWorkbench.vue`、`KnowledgeConfig.vue` | 业务定义与技术数据绑定分区；SQL/字段进入技术区或高级配置 | **第一版完成**；自动映射向导后续增强 |
...
| CORE-015 | P2 | 语义概念中的“动作”与可执行 Ontology Action 容易重复 | `KnowledgeConfig.vue` | 改称“动作词汇（不可执行）”；可执行动作只在 Ontology 动作类型维护 | **已修复** |
...
范围校正：现有 `admin/user` 和用户—验证 Agent 授权只用于当前系统管理、演示和回归。平台面向业务人员与技术人员，正式产品角色待真实业务接入后确定；本轮不新增普通用户独立领域授权，不把验证账号体系扩张为平台主线。
