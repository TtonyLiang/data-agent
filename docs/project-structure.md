...
|---|---|---|---|---|---|
| 1 | 公司级治理与权限 | 业务领域、模型配置、数据访问边界和当前登录门禁 | 业务/技术协作约束、配置 | 可访问范围和运行上下文 | 产品角色是业务人员 + 技术工程师；`admin/user` 仅为已有账号兼容值，不扩建普通用户产品体系 |
...
| 4 | 数据处理与孪生运行时 | 建立对象身份、同步当前状态、关联来源和关系 | 激活模型、`source_query`、业务库数据 | `ontology_object`、`ontology_link`、`twin_sync_run` | 当前支持预览和技术工程师手动分页同步，强制校验版本/数据源漂移 |
...
| 模块 | 负责 | 不负责 |
|---|---|---|
| 公司级治理与权限 | 业务人员/技术工程师职责、业务领域和数据访问边界 | 不创建多企业空间，不定义业务指标 |
...
| `/agent` | 调试与验证智能体 | 仅技术工程师配置本项目内置验证客户端，不是第三方 Agent 注册中心 |
| `/datasource` | 数据源 | 技术工程师测试连接、发现表、采集 Schema |
